# LW003 Android SDK Guide（English）
...
**`com.moko.support.lw003.LoRaLW003MokoSupport`**
...
**`com.moko.support.lw003.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device through `LoRaLW003MokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW003MokoSupport.We recommend putting it in Application.
...
Before operating the Bluetooth scanning device, we need to apply for permission, which we have added in LoRaLW003MokoSupport `AndroidManifest.xml`
...
```
LoRaLW003MokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW003MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW003MokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW003MokoSupport` if you want to use other communication methods.
