package com.moko.support.lw003;
...
public class LoRaLW003MokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW003MokoSupport INSTANCE;

    private Context mContext;

    private LoRaLW003MokoSupport() {
        //no instance
    }

    public static LoRaLW003MokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW003MokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW003MokoSupport();
                }
            }
        }
        return INSTANCE;
